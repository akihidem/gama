...
class MeasurementUnavailable(RuntimeError):
    ...
    **なぜ床を記録側(``_run_one``)に置かないか。** 「0 文字・停止理由なし・error なし」は
    そこで単独に観測できるので、合成物ごとに書くより構造的に見える。実際に既存の trace
    12,126 call を数えると、chars=0 で error の無い call は 150 件あり、**そのうち停止理由を
    名乗っているものは 0 件**だった。ただしこの判定が効くのは「停止理由を名乗れる backend」に
    限られる。名乗らない backend(ollama・CLI 系・試験の stub)では、モデルが本当に空を返した
    場合と区別がつかず、正しい 0 点を測定失敗に化けさせる。判定できない次元で床を張ると
    fail-open ではなく **誤爆**の側に倒れるので、区別できる場所(合成物の中。例外を握って
    いるのは自分)に置く。``SshOpenAIBackend`` が空を返すのは choices を貰えた時だけで、
    その時は停止理由も入っている ── いま使っている backend では穴になっていない。
    """
