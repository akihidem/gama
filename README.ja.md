...
WSL2（CPU のみ ollama）で 56 問プール（28/15/13）を回した実走:

| 世代 | 挑戦者 | search | confirm | 裁定 |
|---|---|---|---|---|
| 0 | `ensemble:qa(llama3.2:3b+qwen2.5-coder:7b)` | 0.607 → 0.571 | 0.733 → 0.733 | 却下（挑戦権を得ていない） |
| 1 | `meshflow:research(llama3.2:3b→qwen2.5-coder:7b)` | 0.607 → 0.548 | 0.667 → 0.800 | 却下（挑戦権を得ていない） |
| 2 | `tool:qa(llama3.2:3b)` | 0.607 → 0.714 | 0.733 → **0.867** | **昇格** |

**封印（13 問・どの判定にも使っていない）: 種 0.577 → チャンピオン 0.846。** confirm で確認した
勝ちが、判定に一度も触れていない case にも出た。伸び幅の内訳も合っていて、sealed 13 問のうち
3 問が `qa`、つまり昇格が変えたクラスそのもの。

この表で 2 つ、ループが効いている証拠がある。1 つは**世代 1 が confirm では上回っていた
（0.800 対 0.667）のに search で負けて却下されている**こと。confirm だけで昇格させる設計なら
これを採っていた。もう 1 つは、**チャンピオンを変えていないのに、その confirm が
0.733 → 0.667 → 0.733 と動いている**こと。これが測定の揺れで、昇格の敷居 δ を定数で決めず
この揺れから読む理由。下限も定数ではなく **confirm 1 問ぶん**（1/n）にしてある。固定の 0.05 は
confirm 8 問なら半問、30 問なら丸 1 問より厳しくなり、**suite を差し替えた瞬間に門の意味が
黙って変わる**。

3 回走らせると、3 回とも同じ 1 手が昇格する。ところが sealed の**全体スコア**は
**+0.269 / +0.186 / +0.019** と散らばり、3 走目は効果が消えたように見える。消えていない。
チャンピオンが差し替えたのは `qa` だけで、それは sealed 13 問中 3 問。残り 10 問の無関係な
ノイズが、どの全体スコアにも平均で混ざっている。実際に変わった case だけを 3 回ずつ測ると、
`qa` は **0.222 → 1.000**、他クラスは動かない。

数字よりこちらが本題で、**全体スコアは、触っていないクラスのノイズで、触ったクラスの変化を
薄める**。1 レーンだけ変えたなら、そのレーンの case を測る。3 走分の数字・case 別の表・
却下された候補は [`recipes/grown-wsl-ollama`](recipes/grown-wsl-ollama) に入れてある。
人が書いていない最初のレシピ。

`wide` を足す前の 26 問プールでは、同じループが 2 回昇格した上で **sealed 5 問で 0.6 → 0.6**
だった。同じループ・同じ箱・同じ規律で、変わったのは答えが見えるだけの case 数があるかどうか。
