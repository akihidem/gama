...
ループは `tool:<クラス>` 変異を 5 本すべて提案し、**5 本とも却下した。どれも confirm を下げた。**
3B で最も再現した結果（`qa → tool`・8 回中 7 回昇格・封印クラスで 0.222 → 1.000）が、48B では
*全クラスで*悪化させる。tool レーンが買うのは「計算できないモデルに計算をさせること」で、
できるモデルには失敗様式が増えるだけの税になる。構造は移植できない。どの構造が自分の箱に効くかを
知る手段がこのループ。
