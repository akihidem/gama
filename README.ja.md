...
段階委譲にする、そして**構造を剥がして素に戻す**）、同じ決定的チェッカで全候補を測り、
...
WSL2（CPU のみ ollama）で全5 suite（86 問・43/23/20 分割）を回した実走:

| 世代 | 挑戦者 | search | confirm | 裁定 |
|---|---|---|---|---|
| 0 | `meshflow:research(3b→coder7b)` | 0.651 → 0.709 | 0.635 → **0.832** | **昇格** |
| 1 | `tool:qa(3b)` | 0.709 → 0.828 | 0.761 → **0.929** | **昇格** |
| 2 | `meshflow:content(3b→coder7b)` | 0.828 → 0.896 | 0.859 → 0.913（+0.054） | 却下（敷居 0.071 未満） |
| 3 | `meshflow:integration(3b→coder7b)` | 0.828 → 0.849 | 0.908 → 0.951（+0.044） | 却下（敷居 0.049 未満） |

**封印（20 問・どの判定にも使っていない）: 0.638 → 0.854。** gama 自身の 2 つのテーゼ（計算は
tool へ、推論は検証 gate 付きの段階委譲へ）を、教えずに機械が拾い直した。そのうえで、confirm
を*実際に改善した*構造変更を 2 つ却下している。改善幅がその世代のチャンピオン自身の測り直しの
揺れより小さかったため。世代 0〜1 は「1 問ぶん」の床が、世代 2〜3 は drift が敷居を決めていて、
`max(1問ぶん, drift)` の両方が同じ走行で仕事をした。

このループの 6 走ぶんを [`recipes/grown-wsl-ollama`](recipes/grown-wsl-ollama) に入れてある。
手法が悪く見える回も含めて全部: 全体スコアが横ばい（+0.019）に出た回（3 問しか触っていない
変更が、無関係な 10 問のノイズで薄まった）、3 走にわたって「ノイズ」と判定していた候補が細かい
split で本物と分かった件、そして**最有力の変更を一度も提案しないまま終わった回**（探索幅が狭く、
別の候補がキュー先頭に居座っていた）。台帳上、不在は却下と同じ顔をする。台帳を残す理由がそこ。
