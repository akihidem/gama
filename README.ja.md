...
「分離できない」は一語で二つの結果を指していて、ループはいまどちらかを言う。走行が `confirm`
で主張した伸びを sealed の問数に直すと: run V は +1.1/65 問 → sealed 32 問なら 0.54 問で、
sealed が分解する 1 問の内側。判定は**封を開ける前から決まっていた**(run T も同じ: +1.25/56 →
0.62/28)。run R は +4.37/48 → sealed 24 問で 2.19 問見えるはずが +0.33 問。こちらは confirm の
伸びが**転移しなかった**＝昇格は選択の上振れだった。前者は sealed を広げる(か大きな手を通す)
話で、後者は昇格を疑う話。走行は種の時点で sealed の分解能を confirm の問数で印字するので、
