...
WSL2（CPU のみ ollama）で 9 走まわし（最大 86 問・43/23/20 分割）、構造的な変更を 2 つ見つけて、
決着したのは 1 つだけだった。`qa → tool` は**提案された 8 走のうち 7 走で昇格**。case 別に測ると
封印された `qa` 3 問が 0.222 → 1.000 になり、そこにはプールのどのモデルも素では解けない問題も
含まれる（3 時間 45 分を 15300 秒と答える。同じ 3B に「プログラムで」と言うと `3*3600+45*60` と
書く）。`research → meshflow` は 3 勝 6 敗で、最小プールで勝ち、最大プールで負けた。

この 2 つ目について、本プロジェクトは「ノイズ」と判定し、撤回して「本物」とし、それも撤回した。
どの反転も 1〜2 走にパターンを読み込んだせいで、安定した言い方には 9 走が要った。だからレシピは
**再現した方だけを載せ**、もう一方は記録ごと書いてある:
